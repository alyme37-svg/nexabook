...
function NavigationLink({ item }: { item: (typeof allNavigation)[number] }) {
  ...
  return (
    ...
    >
      {active ? (
        <span className="absolute inset-y-2 -left-0.5 w-0.5 rounded-full bg-primary shadow-[0_0_10px_rgb(99_102_241/0.7)]" />
      ) : null}
...
export function AppSidebar() {
  ...
  const businessName = useNexaBookStore(
    (state) => state.businessSettings.businessName,
  );
  ...
  return (
    <aside className="sticky top-0 hidden h-dvh w-60 flex-col border-r bg-card px-3 py-4 lg:flex">
      <Link
        href="/dashboard"
        className="mx-2 mb-7 rounded-lg outline-none focus-visible:ring-3 focus-visible:ring-ring/25"
      >
        <Brand />
      ...
      <nav
        ...
        className="mt-auto grid gap-1 border-t pt-3"
      >
      ...
      <div className="mt-3 flex items-center gap-3 rounded-xl border bg-muted/35 p-2.5">
        <span
          className="grid size-9 place-items-center rounded-lg bg-[linear-gradient(135deg,#3730a3,#7c3aed)] text-xs font-bold text-white"
          aria-hidden="true"
        >
          {initials}
        </span>
      ...
      <p className="mt-3 px-2 text-center text-[10px] leading-4 text-muted-foreground/70">
        Portfolio demo by Ali Elhussein
      </p>
    </aside>
  );
}
